...
**Transport backend auto-selection.** `EdlOptions.Backend = Auto` resolves to **LibUsb on Windows *and* macOS** and Serial on Linux. macOS picks LibUsb by default because the IOKit-backed `/dev/tty.usbserial-*` path requires root; libusb opens the USB interface directly via IOKit without elevation. `ElevationPolicy.RequiresHelper(TransportBackend)` short-circuits to `false` for LibUsb so the privileged helper is only spawned for the macOS serial path.
